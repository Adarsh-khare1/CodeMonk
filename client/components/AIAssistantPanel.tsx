...
export default function AIAssistantPanel({
  ...
  return (
    <div className="surface-primary space-y-5 p-6">
      <div>
        <h2 className="text-xl font-bold tracking-tight">AI Assistant</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Review your code, generate a cleaner version, or ask problem-specific questions.
        </p>
      </div>
      ...
      {review && (
        <div className="space-y-4 rounded-2xl border border-border/70 bg-secondary/40 p-4">
          ...
          {review.edgeCases?.length > 0 && (
            <div>
              <h4 className="font-medium text-sky-300">Edge Cases</h4>
              <ul className="mt-2 space-y-2 text-sm text-foreground">
      ...
      {optimization && (
        <div className="space-y-4 rounded-2xl border border-border/70 bg-secondary/40 p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={() => onApplyOptimizedCode(optimization.optimizedCode)}
          ...
          {optimization.changes?.length > 0 && (
            <div>
              ...
              <ul className="mt-2 space-y-2 text-sm text-foreground">
                {optimization.changes.map((item, index) => (
                  <li key={index}>• {item}</li>
                ))}
              </ul>
            </div>
          ...
          {optimization.tradeoffs?.length > 0 && (
            <div>
              <h4 className="font-medium text-amber-300">Tradeoffs</h4>
              <ul className="mt-2 space-y-2 text-sm text-foreground">
                {optimization.tradeoffs.map((item, index) => (
                  <li key={index}>• {item}</li>
                ))}
              </ul>
            </div>
          ...
          {optimization.warnings?.length > 0 && (
            <div>
              <h4 className="font-medium text-red-600 dark:text-red-300">Warnings</h4>
              <ul className="mt-2 space-y-2 text-sm text-foreground">
                {optimization.warnings.map((item, index) => (
                  <li key={index}>• {item}</li>
                ))}
              </ul>
            </div>
      ...
      <div className="space-y-3">
        ...
        <div className="max-h-80 space-y-3 overflow-y-auto rounded-2xl border border-border/70 bg-secondary/40 p-4">
        ...
        <div className="flex gap-3">
          <textarea
            ...
            className="input-surface min-h-[88px] flex-1 px-3 py-2 text-sm"
            placeholder="Ask Gemini about your code, bugs, edge cases, or approach..."
          />
          <button
            onClick={handleChat}
            disabled={loadingAction !== null || isCodeEmpty || !chatInput.trim()}
            className="self-end rounded-xl bg-sky-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-sky-700 disabled:opacity-50"
          >
